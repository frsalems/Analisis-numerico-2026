...
Para utilizar las regletas interactivas, descarga el archivo y ábrelo desde Pluto.
...
x2 = F \ b2
```
### Laboratorio: nuestro solucionador LU

Antes de estudiar el pivoteo, trabajaremos con nuestra propia implementación
de la factorización LU y las sustituciones hacia adelante y hacia atrás.

👉 [**Abrir laboratorio LU en Pluto**](lu_sin_linearalgebra_pluto.jl)

En este cuaderno resolveremos sistemas lineales sin utilizar `A \ b` ni
la factorización LU de `LinearAlgebra`.

> **Experimento:** predice → ejecuta → observa → explica.
...
El pivoteo parcial puede incorporarse directamente al proceso de
eliminación. En cada etapa \(k\), antes de eliminar los elementos situados
debajo del pivote, buscamos en la columna \(k\) el elemento de mayor
magnitud entre las filas \(k,\ldots,n\).

Si este elemento se encuentra en una fila distinta de \(k\), intercambiamos
las dos filas y continuamos la eliminación.

Los intercambios realizados pueden representarse mediante una matriz de
permutación \(P\). Por esta razón, la factorización obtenida ya no tiene
la forma
...
En forma esquemática, para \(k=1,\ldots,n-1\):

1. Buscar

   $$
   p=\mathop{\mathrm{arg\,max}}_{i=k,\ldots,n}|u_{ik}|.
   $$

2. Intercambiar las filas \(k\) y \(p\) de \(U\).

3. Registrar el mismo intercambio en la matriz de permutación \(P\).

4. Si ya se han calculado multiplicadores en etapas anteriores,
   intercambiar también las entradas correspondientes de \(L\).

5. Para \(i=k+1,\ldots,n\), calcular

   $$
   m_{ik}=\frac{u_{ik}}{u_{kk}},
   $$

   guardar \(m_{ik}\) en \(L\) y efectuar

   $$
   R_i\leftarrow R_i-m_{ik}R_k.
   $$

Al terminar, \(L\) es triangular inferior, \(U\) es triangular superior
y se cumple
...
### Nuestra implementación en Julia

Construiremos nuestra propia función

```julia
mi_LU_pivoteo(A)
```

que devolverá las tres matrices

```julia
P, L, U
```

sin utilizar una factorización LU proporcionada por `LinearAlgebra`.

Una vez obtenida la factorización, para resolver
...
Por tanto, resolveremos sucesivamente

$$
Ly=Pb
$$

mediante sustitución hacia adelante, y
...
En el laboratorio de Pluto construiremos primero una factorización
\(PA=LU\) a mano para una matriz \(3\times3\). Después haremos que
`mi_LU_pivoteo(A)` realice automáticamente el mismo trabajo.
👉 [**Explorar la factorización LU con pivoteo en Pluto**](lu_con_pivoteo_pluto.jl)
...
Queremos estudiar experimentalmente cómo crece el tiempo de ejecución
de la multiplicación de una matriz por un vector cuando aumenta el
tamaño del problema.

Usaremos la siguiente implementación:

```julia
function matporvec(A, x)
    n, m = size(A)
...
```
Ahora realice el experimento para
...
1. Genere una matriz aleatoria $A$ de tamaño $n\times n$ y un vector
   aleatorio $x$ de longitud $n$.

2. Mida el tiempo de ejecución de

   ```julia
   matporvec(A,x)
   ```

3. Registre los tiempos obtenidos.

4. Para cada par de tamaños consecutivos, calcule
...
Compare los valores obtenidos de $p$.

- ¿Se acercan a $2$?
- ¿Por qué esperaríamos un valor cercano a $2$ al observar el código de
  `matporvec`?
- ¿Por qué los valores experimentales no tienen que ser exactamente
  iguales a $2$?
